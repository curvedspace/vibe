...
void Vt102Emulation::tau( int token, int p, int q )
{
    ...
    switch( N ) {
        ...
        case 2:
            ...
            break;
        case 3:
            printf("ESC_CS-%c-%c ", (token>>8)&0xff, (token>>16)&0xff);
            break;
        case 4:
            printf("ESC_DE-%c ", (token>>8)&0xff);
            break;
        case 5:
            printf("CSI-PS-%c-%d", (token>>8)&0xff, (token>>16)&0xff );
            break;
        case 6:
            printf("CSI-PN-%c [%d]", (token>>8)&0xff, p);
            break;
        case 7:
            printf("CSI-PR-%c-%d", (token>>8)&0xff, (token>>16)&0xff );
            break;
        case 8:
            printf("VT52-%c", (token>>8)&0xff);
            break;
        case 9:
            printf("CSI-PG-%c", (token>>8)&0xff);
            break;
        case 10:
            printf("CSI-PE-%c", (token>>8)&0xff);
            break;
    }
#endif
    ...
    switch (token) {
        ...
        case TY_CHR(         ) :
            _currentScreen->ShowCharacter        (p         );
            break; //UTF16
        ...
        case TY_CTL('W'      ) : /* ETB: ignored                      */
            break;
        case TY_CTL('X'      ) :
            _currentScreen->ShowCharacter        (    0x2592);
            break; //VT100
