...
struct VCategorizedView::Private::Block {
    Block()
        : topLeft(QPoint())
        , height(-1)
...
VCategorizedView::Private::Private(VCategorizedView *q)
    : q(q)
    , proxyModel(0)
    , categoryDrawer(0)
    , categorySpacing(5)
...
VCategorizedView::Private::~Private()
{
    delete hoveredBlock;
}

bool VCategorizedView::Private::isCategorized() const
{
    return proxyModel && categoryDrawer && proxyModel->isCategorizedModel();
}

QStyleOptionViewItemV4 VCategorizedView::Private::blockRect(const QModelIndex &representative)
{
    QStyleOptionViewItemV4 option(q->viewOptions());
    const int height = categoryDrawer->categoryHeight(representative, option);
    const QString categoryDisplay = representative.data(VCategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
...
QPair<QModelIndex, QModelIndex> VCategorizedView::Private::intersectingIndexesWithRect(const QRect &_rect) const
{
...
QPoint VCategorizedView::Private::blockPosition(const QString &category)
{
...
int VCategorizedView::Private::blockHeight(const QString &category)
{
    Block &block = blocks[category];
...
int VCategorizedView::Private::viewportWidth() const
{
    return q->viewport()->width() - categorySpacing * 2 - categoryDrawer->leftMargin() - categoryDrawer->rightMargin();
}

void VCategorizedView::Private::regenerateAllElements()
{
...
void VCategorizedView::Private::rowsInserted(const QModelIndex &parent, int start, int end)
{
    if (!isCategorized()) {
        return;
    }
...
QRect VCategorizedView::Private::mapToViewport(const QRect &rect) const
{
...
QRect VCategorizedView::Private::mapFromViewport(const QRect &rect) const
{
    const int dx = q->horizontalOffset();
    const int dy = q->verticalOffset();
    return rect.adjusted(dx, dy, dx, dy);
...
int VCategorizedView::Private::highestElementInLastRow(const Block &block) const
{
...
bool VCategorizedView::Private::hasGrid() const
{
    const QSize gridSize = q->gridSize();
    return gridSize.isValid() && !gridSize.isNull();
}

QString VCategorizedView::Private::categoryForIndex(const QModelIndex &index) const
{
    const QModelIndex categoryIndex = index.model()->index(index.row(), proxyModel->sortColumn(), index.parent());
    return categoryIndex.data(VCategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
}

void VCategorizedView::Private::leftToRightVisualRect(const QModelIndex &index, Item &item,
...
void VCategorizedView::Private::topToBottomVisualRect(const QModelIndex &index, Item &item,
                                                      const Block &block, const QPoint &blockPos) const
{
    const int firstIndexRow = block.firstIndex.row();
...
void VCategorizedView::Private::_k_slotCollapseOrExpandClicked(QModelIndex)
{
}
...
VCategorizedView::VCategorizedView(QWidget *parent)
    : QListView(parent)
    , d(new Private(this))
{
}

VCategorizedView::~VCategorizedView()
{
    delete d;
}

void VCategorizedView::setModel(QAbstractItemModel *model)
{
    ...
    d->proxyModel = dynamic_cast<VCategorizedSortFilterProxyModel *>(model);
...
void VCategorizedView::setGridSize(const QSize &size)
{
    setGridSizeOwn(size);
}

void VCategorizedView::setGridSizeOwn(const QSize &size)
{
    d->regenerateAllElements();
    QListView::setGridSize(size);
}

QRect VCategorizedView::visualRect(const QModelIndex &index) const
{
    if (!d->isCategorized()) {
...
VCategoryDrawer *VCategorizedView::categoryDrawer() const
{
    return d->categoryDrawer;
}

void VCategorizedView::setCategoryDrawer(VCategoryDrawer *categoryDrawer)
{
    if (d->categoryDrawer)
...
int VCategorizedView::categorySpacing() const
{
    return d->categorySpacing;
}

void VCategorizedView::setCategorySpacing(int categorySpacing)
{
...
bool VCategorizedView::alternatingBlockColors() const
{
    return d->alternatingBlockColors;
}

void VCategorizedView::setAlternatingBlockColors(bool enable)
{
    d->alternatingBlockColors = enable;
}

bool VCategorizedView::collapsibleBlocks() const
{
    return d->collapsibleBlocks;
}

void VCategorizedView::setCollapsibleBlocks(bool enable)
{
    d->collapsibleBlocks = enable;
}

QModelIndexList VCategorizedView::block(const QString &category)
{
    QModelIndexList res;
...
QModelIndexList VCategorizedView::block(const QModelIndex &representative)
{
    return block(representative.data(VCategorizedSortFilterProxyModel::CategoryDisplayRole).toString());
}

QModelIndex VCategorizedView::indexAt(const QPoint &point) const
{
    if (!d->isCategorized()) {
...
void VCategorizedView::reset()
{
    d->blocks.clear();
    QListView::reset();
}

void VCategorizedView::paintEvent(QPaintEvent *event)
{
    if (!d->isCategorized()) {
    ...
    if (intersecting.first.isValid() && intersecting.second.isValid()) {
        ...
        while (i <= intersecting.second.row()) {
            ...
            if (i == indexToCheckIfBlockCollapsed) {
                ...
                category = categoryIndex.data(VCategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
...
void VCategorizedView::resizeEvent(QResizeEvent *event)
{
    d->regenerateAllElements();
    QListView::resizeEvent(event);
}

void VCategorizedView::setSelection(const QRect &rect,
...
void VCategorizedView::mouseMoveEvent(QMouseEvent *event)
{
...
void VCategorizedView::mousePressEvent(QMouseEvent *event)
{
...
void VCategorizedView::mouseReleaseEvent(QMouseEvent *event)
{
...
void VCategorizedView::leaveEvent(QEvent *event)
{
...
void VCategorizedView::startDrag(Qt::DropActions supportedActions)
{
    QListView::startDrag(supportedActions);
}

void VCategorizedView::dragMoveEvent(QDragMoveEvent *event)
{
    QListView::dragMoveEvent(event);
    d->hoveredIndex = indexAt(event->pos());
}

void VCategorizedView::dragEnterEvent(QDragEnterEvent *event)
{
    QListView::dragEnterEvent(event);
}

void VCategorizedView::dragLeaveEvent(QDragLeaveEvent *event)
{
    QListView::dragLeaveEvent(event);
}

void VCategorizedView::dropEvent(QDropEvent *event)
{
...
QModelIndex VCategorizedView::moveCursor(CursorAction cursorAction,
...
void VCategorizedView::rowsAboutToBeRemoved(const QModelIndex &parent,
                                            int start,
                                            int end)
{
    if (!d->isCategorized()) {
...
void VCategorizedView::updateGeometries()
{
     ...
     *
     * As a result QListView and VCategorizedView occasionally started a race on the scrollbar visibility, effectively blocking the UI
    ...
    //      In theory, as VCategorizedView has been designed, there is no need of horizontal scroll bar.
    horizontalScrollBar()->setRange(0, 0);
...
void VCategorizedView::currentChanged(const QModelIndex &current,
                                      const QModelIndex &previous)
{
    QListView::currentChanged(current, previous);
}

void VCategorizedView::dataChanged(const QModelIndex &topLeft,
...
void VCategorizedView::rowsInserted(const QModelIndex &parent,
                                    int start,
                                    int end)
{
...
void VCategorizedView::slotLayoutChanged()
{
    if (!d->isCategorized())
        return;
...
#include "moc_vcategorizedview.cpp"
