...
VBookmark VBookmarkGroup::addBookmark(const QString &text, const QUrl &url, const QString &icon)
{
    ...
    VBookmark newVBookmark = addBookmark(VBookmark(elem));

    // as icons are moved to metadata, we have to use the VBookmark API for this
    newVBookmark.setIcon(icon.isEmpty() ? VMimeType::iconNameForUrl(url) : icon);
    return newVBookmark;
}
...
VBookmark::VBookmark(const QDomElement &elem) : element(elem)
{
}
...
{
    qDebug() << "VBookmark::updateAccessMetadata " << address() << " " << url().toString();
...
void VBookmark::List::populateMimeData(QMimeData *mimeData) const
{
    QList<QUrl> urls;
...
bool VBookmark::List::canDecode(const QMimeData *mimeData)
{
...
QStringList VBookmark::List::mimeDataTypes()
{
...
VBookmark::List VBookmark::List::fromMimeData(const QMimeData *mimeData, QDomDocument &doc)
{
    VBookmark::List bookmarks;
