...
VDeviceInterface::VDeviceInterface(VDeviceInterfacePrivate &dd, QObject *backendObject)
    : d_ptr(&dd)
{
    Q_D(VDeviceInterface);

    d->setBackendObject(backendObject);
}

VDeviceInterface::~VDeviceInterface()
{
    delete d_ptr;
    d_ptr = 0;
}

bool VDeviceInterface::isValid() const
{
    Q_D(const VDeviceInterface);
    return d->backendObject() != 0;
}

QString VDeviceInterface::typeToString(Type type)
{
...
VDeviceInterface::Type VDeviceInterface::stringToType(const QString &type)
{
    int index = staticMetaObject.indexOfEnumerator("Type");
    QMetaEnum metaEnum = staticMetaObject.enumerator(index);
...
QString VDeviceInterface::typeDescription(Type type)
{
    switch (type) {
        case Unknown:
...
VDeviceInterfacePrivate::VDeviceInterfacePrivate()
    : m_devicePrivate(0)
{
}
...
QObject *VDeviceInterfacePrivate::backendObject() const
{
    return m_backendObject.data();
}

void VDeviceInterfacePrivate::setBackendObject(QObject *object)
{
    m_backendObject = object;
}

VDevicePrivate *VDeviceInterfacePrivate::devicePrivate() const
{
    return m_devicePrivate;
}

void VDeviceInterfacePrivate::setDevicePrivate(VDevicePrivate *devicePrivate)
{
