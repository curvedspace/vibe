...
namespace VHardware
{
    namespace Backends
    {
        namespace UPnP
        {
            ...
            bool UPnPDevice::queryDeviceInterface(const VDeviceInterface::Type &type) const
            {
                ...
                } else if (type == VDeviceInterface::InternetGateway) {
            ...
            QObject *UPnPDevice::createDeviceInterface(const VDeviceInterface::Type &type)
            {
                if (type == VDeviceInterface::StorageAccess) {
                    if (isMediaServer()) {
                        return new VHardware::Backends::UPnP::UPnPMediaServer(this);
                    } else {
                        return 0;
                    }
                } else if (type == VDeviceInterface::InternetGateway) {
                    if (isInternetGatewayDevice()) {
                        return new VHardware::Backends::UPnP::UPnPInternetGateway(this);
                    }
                }
