...
#define ASM_MOV_VAR(var, reg)     "movq   "var",     %%r"reg"  \n\t"
            ...
            QubeHardware::Processor::InstructionSets cpuFeatures()
            {
                ...
                QubeHardware::Processor::InstructionSets featureflags;

                if (features & 0x80000000)
                    featureflags |= QubeHardware::Processor::Amd3DNow;
                if (features & 0x00800000)
                    featureflags |= QubeHardware::Processor::IntelMmx;
                if (features & 0x02000000)
                    featureflags |= QubeHardware::Processor::IntelSse;
                if (features & 0x04000000)
                    featureflags |= QubeHardware::Processor::IntelSse2;
                if (features & 0x00000001) // FIXME: Only SSE3. There is no flag for SSSE3.
                    featureflags |= QubeHardware::Processor::IntelSse3;
                if (features & 0x00080000) // FIXME: Only SSE4.1. There is no flag for SSE4.2.
                    featureflags |= QubeHardware::Processor::IntelSse4;

                if (features & 0x2)
                    featureflags |= QubeHardware::Processor::AltiVec;
