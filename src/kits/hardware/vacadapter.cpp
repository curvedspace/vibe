...
#include "vacadapter.h"
#include "vacadapter_p.h"
#include "vcall_p.h"

VAcAdapter::VAcAdapter(QObject *backendObject)
    : VDeviceInterface(*new VAcAdapterPrivate(), backendObject)
{
    connect(backendObject, SIGNAL(plugStateChanged(bool, const QString &)),
            this, SIGNAL(plugStateChanged(bool, const QString &)));
}

VAcAdapter::~VAcAdapter()
{
}

bool VAcAdapter::isPlugged() const
{
    Q_D(const VAcAdapter);
