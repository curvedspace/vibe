...
namespace Qube
{
    namespace Core
    {
        ...
        \brief The CommandOptions class is a Parser for command-line options
        ...
        ("--option") options. By default, CommandOptions uses Windows-style options
        (CommandOptions::Slash) on Windows and GNU-style options (CommandOptions::DoubleDash)
        ...
        To use CommandOptions, first add the supported options using the add() and alias()
        ...
        options.add("level", "set the compression level (0-9)", CommandOptions::Required);
        options.alias("level", "l");
        options.add("verbose", "show more information about the process; specify twice for more detail", CommandOptions::AllowMultiple);
        ...
        */
        ...
        static const char* qxt_qt_options[] = {
            "=style",       QT_TRANSLATE_NOOP("CommandOptions", "sets the application GUI style"),
            "=stylesheet",  QT_TRANSLATE_NOOP("CommandOptions", "sets the application stylesheet"),
            "=session",     QT_TRANSLATE_NOOP("CommandOptions", "restores the application from an earlier session"),
            "widgetcount",  QT_TRANSLATE_NOOP("CommandOptions", "displays debugging information about widgets"),
            "reverse",      QT_TRANSLATE_NOOP("CommandOptions", "use right-to-left layout"),
#ifdef QT_DEBUG
            "nograb",       QT_TRANSLATE_NOOP("CommandOptions", "never grab the mouse or keyboard"),
#endif
#if defined(QT_DEBUG) && defined(Q_WS_X11)
            "dograb",       QT_TRANSLATE_NOOP("CommandOptions", "grab the mouse/keyboard even in a debugger"),
            "sync",         QT_TRANSLATE_NOOP("CommandOptions", "run in synchronous mode for debugging"),
#endif
#ifdef Q_WS_WIN
            "direct3d",     QT_TRANSLATE_NOOP("CommandOptions", "use Direct3D by default"),
#endif
#ifdef Q_WS_X11
            "=display",     QT_TRANSLATE_NOOP("CommandOptions", "sets the X11 display"),
            "=geometry",    QT_TRANSLATE_NOOP("CommandOptions", "sets the geometry of the first window"),
            "=font",        "",
            "=fn",          QT_TRANSLATE_NOOP("CommandOptions", "sets the default font"),
            "=background",  "",
            "=bg",          QT_TRANSLATE_NOOP("CommandOptions", "sets the default background color"),
            "=foreground",  "",
            "=fg",          QT_TRANSLATE_NOOP("CommandOptions", "sets the default foreground color"),
            "=button",      "",
            "=btn",         QT_TRANSLATE_NOOP("CommandOptions", "sets the default button color"),
            "=name",        QT_TRANSLATE_NOOP("CommandOptions", "sets the application name"),
            "=title",       QT_TRANSLATE_NOOP("CommandOptions", "sets the application title"),
            "=visual",      QT_TRANSLATE_NOOP("CommandOptions", "sets the X11 visual type"),
            "=ncols",       QT_TRANSLATE_NOOP("CommandOptions", "limit the number of colors on an 8-bit display"),
            "cmap",         QT_TRANSLATE_NOOP("CommandOptions", "use a private color map"),
            "=im",          QT_TRANSLATE_NOOP("CommandOptions", "sets the input method server"),
            "noxim",        QT_TRANSLATE_NOOP("CommandOptions", "disable the X Input Method"),
            "=inputstyle",  QT_TRANSLATE_NOOP("CommandOptions", "sets the style used by the input method"),
#endif
            0,              0
        };
...
// Storage structure for option data
        struct CommandOption {
            ...
            CommandOptions::ParamTypes paramType;    // flags
            quint16 group;          // mutual exclusion group
        };

        class CommandOptionsPrivate
        {
            Q_DECLARE_PUBLIC(CommandOptions)
        ...
        public:
            ...
            CommandOptions::FlagStyle flagStyle;
            CommandOptions::ParamStyle paramStyle;
            ...
            CommandOptionsPrivate(CommandOptions *q);
        ...
        private:
            CommandOptions *q_ptr;
        };

        CommandOptionsPrivate::CommandOptionsPrivate(CommandOptions *q) :
            q_ptr(q)
        {
        }
        ...
        CommandOption* CommandOptionsPrivate::findOption(const QString& name)
        {
            ...
            qWarning() << qPrintable(QString("CommandOptions: ") + tr("option \"%1\" not found").arg(name));
            return 0;
        }
        ...
        /*!
         * Constructs a CommandOptions object.
         */
        CommandOptions::CommandOptions() :
            d_ptr(new CommandOptionsPrivate(this))
        {
         ...
         */
        void CommandOptions::setFlagStyle(FlagStyle style)
        {
            Q_D(CommandOptions);
            d->flagStyle = style;
        }
         ...
         */
        CommandOptions::FlagStyle CommandOptions::flagStyle() const
        {
            Q_D(const CommandOptions);
            return d->flagStyle;
        }
         ...
         * this setting.
         */
        void CommandOptions::setParamStyle(ParamStyle style)
        {
            Q_D(CommandOptions);
            d->paramStyle = style;
        }
         ...
         * Sets which value separator is used for options that accept parameters.
         */
        CommandOptions::ParamStyle CommandOptions::paramStyle() const
        {
            Q_D(const CommandOptions);
            return d->paramStyle;
        }
         ...
         */
        void CommandOptions::setScreenWidth(quint16 width)
        {
            Q_D(CommandOptions);
            d->screenWidth = width;
        }
         ...
         */
        quint16 CommandOptions::screenWidth() const
        {
            Q_D(const CommandOptions);
         ...
         * options.
         */
        void CommandOptions::addSection(const QString& name)
        {
            Q_D(CommandOptions);
         ...
         */
        void CommandOptions::add(const QString& name, const QString& desc, ParamTypes paramType, int group)
        {
            Q_D(CommandOptions);
         ...
         */
        void CommandOptions::alias(const QString& from, const QString& to)
        {
            Q_D(CommandOptions);
            ...
            if (option->paramType & ValueOptional && d->flagStyle == DoubleDash && to.length() == 1)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("Short options cannot have optional parameters"));
        }
         ...
         * \sa flagStyle()
         */
        QStringList CommandOptions::positional() const
        {
            Q_D(const CommandOptions);

            if (!d->parsed)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("positional() called before parse()"));
            return d->positional;
        }
         ...
         */
        QStringList CommandOptions::unrecognized() const
        {
            Q_D(const CommandOptions);

            if (!d->parsed)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("unrecognized() called before parse()"));
         ...
         * CommandOptions::AllowMultiple flag set.
         */
        int CommandOptions::count(const QString& name) const
        {
            Q_D(const CommandOptions);

            if (!d->parsed)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("count() called before parse()"));
         ...
         * If the option was created with the CommandOptions::AllowMultiple flag, and the option
         ...
         * in the QVariant.
         */
        QVariant CommandOptions::value(const QString& name) const
        {
            Q_D(const CommandOptions);

            if (!d->parsed)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("value() called before parse()"));
            const CommandOption* option = d->findOption(name);
            if (!option)
         ...
         */
        QMultiHash<QString, QVariant> CommandOptions::parameters() const
        {
            Q_D(const CommandOptions);

            if (!d->parsed)
                qWarning() << qPrintable(QString("CommandOptions: ") + tr("parameters() called before parse()"));
         ...
         * \sa QCoreApplication::arguments()
         */
        void CommandOptions::parse(QStringList params)
        {
            Q_D(CommandOptions);
            d->parse(params);
            d->parsed = true;
        ...
        void CommandOptionsPrivate::setOption(CommandOption* option, const QString& value)
        {
            ...
            if (!(option->paramType & CommandOptions::AllowMultiple))
                option->values.clear();
        ...
        void CommandOptionsPrivate::parse(const QStringList& params)
        {
            ...
            while (pos < ct) {
                ...
                if (!endFlags && ((flagStyle == CommandOptions::Slash && param[0] == '/') ||
                                  (flagStyle != CommandOptions::Slash && param[0] == '-'))) {
                    // tagged argument
                    if (param.length() == 1) {
                    ...
                    if (flagStyle != CommandOptions::Slash && param == "--") {
                        // End of parameters flag
                        endFlags = true;
                    } else if (flagStyle == CommandOptions::DoubleDash && param[1] != '-') {
                        ...
                        for (int i = 1; i < len; i++) {
                            ...
                            if (ch == "-") {
                                ...
                                if (!option) {
                                    ...
                                    if (option->paramType & CommandOptions::ValueRequired) {
                        ...
                        if (flagStyle == CommandOptions::DoubleDash)
                            name = param.mid(2);
                        else
                            name = param.mid(1);
                        ...
                        if (!option) {
                            ...
                            if (option->paramType & CommandOptions::ValueRequired && !hasEquals) {
                                ...
                                value = params[pos];
                                pos++;
                            } else if ((paramStyle & CommandOptions::Space)
                                       && (option->paramType & CommandOptions::ValueOptional)
                                       && !hasEquals) {
                                if (pos < params.count()) {
                                    if (!((flagStyle == CommandOptions::Slash && params.at(pos)[0] == '/') ||
                                          (flagStyle != CommandOptions::Slash && params.at(pos)[0] == '-'))) {
                                        value = params[pos];
                                        pos++;
                                    }
                                }
         ...
         */
        bool CommandOptions::showUnrecognizedWarning(QIODevice* device) const
        {
            if (!device) {
         ...
         * \sa QCoreApplication::applicationFilePath()
         */
        QString CommandOptions::getUnrecognizedWarning() const
        {
            QString usage;
         ...
         * \sa QCoreApplication::applicationFilePath()
         */
        bool CommandOptions::showUnrecognizedWarning(QTextStream& stream) const
        {
            Q_D(const CommandOptions);
         ...
         * \sa QApplication
         */
        void CommandOptions::showUsage(bool showQtOptions, QIODevice* device) const
        {
            if (!device) {
         ...
         */
        QString CommandOptions::getUsage(bool showQtOptions) const
        {
            QString usage;
            QTextStream stream(&usage);
            showUsage(showQtOptions, *&stream);
         ...
         * \sa QApplication
         */
        void CommandOptions::showUsage(bool showQtOptions, QTextStream& stream) const
        {
            Q_D(const CommandOptions);
