...
namespace Qube
{
    namespace Core
    {
        ...
        void SaveFile::setFileName(const QString &filename)
        {
        ...
        QFile::FileError SaveFile::error() const
        {
        ...
        QString SaveFile::errorString() const
        {
        ...
        QString SaveFile::fileName() const
        {
...
#  define FDATASYNC fsync
        ...
        bool SaveFile::finalize()
        {
            ...
            if (!d->wasFinalized) {
                ...
                if (extraSync < 0)
                    extraSync = getenv("QUBE_EXTRA_FSYNC") != 0 ? 1 : 0;
                if (extraSync) {
                    if (flush()) {
                        forever {
                            if (!FDATASYNC(handle()))
...
#undef FDATASYNC
        ...
        bool SaveFile::backupFile(const QString& qFilename, const QString& backupDir)
        {
        ...
        bool SaveFile::simpleBackupFile(const QString& qFilename,
                                        ...
                                        const QString& backupExtension)
        {
            ...
            //qDebug() << "SaveFile copying " << qFilename << " to " << backupFileName;
            QFile::remove(backupFileName);
            return QFile::copy(qFilename, backupFileName);
        }

        bool SaveFile::numberedBackupFile(const QString& qFilename,
                                          const QString& backupDir,
                                          const QString& backupExtension,
                                          const uint maxBackups)
        {
            ...
            for (int i=maxBackupFound; i>0; i--) {
                ...
                //qDebug() << "SaveFile renaming " << from << " to " << to;
                QFile::rename(from, to);
                to = from;
            }

            // Finally create most recent backup by copying the file to backup number 1.
            //qDebug() << "SaveFile copying " << qFilename << " to " << sTemplate.arg(1);
