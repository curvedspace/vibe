...
class VGzipCompressionFilter::Private
{
public:
    Private()
...
VGzipCompressionFilter::VGzipCompressionFilter()
    : d(new Private)
{
}
...
void VGzipCompressionFilter::init(int mode)
{
    init(mode, filterFlags() == WithHeaders ? GZipHeader : RawDeflate);
}

void VGzipCompressionFilter::init(int mode, Flag flag)
{
    if (d->isInitialized) {
        terminate();
    }
    ...
    if (mode == QIODevice::ReadOnly) {
        ...
        qWarning() << "VGzipCompressionFilter: Unsupported mode " << mode << ". Only QIODevice::ReadOnly and QIODevice::WriteOnly supported";
    }
    d->mode = mode;
    d->compressed = true;
...
int VGzipCompressionFilter::mode() const
{
    return d->mode;
}

void VGzipCompressionFilter::terminate()
{
...
void VGzipCompressionFilter::reset()
{
    if (d->mode == QIODevice::ReadOnly) {
...
bool VGzipCompressionFilter::readHeader()
{
...
bool VGzipCompressionFilter::writeHeader(const QByteArray &fileName)
{
...
void VGzipCompressionFilter::writeFooter()
{
...
void VGzipCompressionFilter::setOutBuffer(char *data, uint maxlen)
{
    d->zStream.avail_out = maxlen;
    d->zStream.next_out = (Bytef *) data;
}
void VGzipCompressionFilter::setInBuffer(const char *data, uint size)
{
#ifdef DEBUG_GZIP
...
int VGzipCompressionFilter::inBufferAvailable() const
{
    return d->zStream.avail_in;
}
int VGzipCompressionFilter::outBufferAvailable() const
{
    return d->zStream.avail_out;
}

VGzipCompressionFilter::Result VGzipCompressionFilter::uncompress_noop()
{
...
VGzipCompressionFilter::Result VGzipCompressionFilter::uncompress()
{
#ifndef NDEBUG
    if (d->mode == 0) {
        qWarning() << "mode==0; VGzipCompressionFilter::init was not called!";
...
#endif
        return (result == Z_OK ? VAbstractCompressionFilter::Ok : (result == Z_STREAM_END ? VAbstractCompressionFilter::End : VAbstractCompressionFilter::Error));
    } else
        return uncompress_noop();
}

VGzipCompressionFilter::Result VGzipCompressionFilter::compress(bool finish)
{
    ...
    VGzipCompressionFilter::Result callerResult = result == Z_OK ? VAbstractCompressionFilter::Ok : (Z_STREAM_END ? VAbstractCompressionFilter::End : VAbstractCompressionFilter::Error);
    ...
    if (result == Z_STREAM_END && d->headerWritten && !d->footerWritten) {
        if (d->zStream.avail_out >= 8 /*footer size*/) {
            ...
            callerResult = VAbstractCompressionFilter::Ok;
        }
    }
    return callerResult;
}
