...
static const bool kAnimateBusyProgressBar = true;
static const bool kAnimateProgressBar = false;
static const int kProgressBarFps = 25;

// Width of line edit focus frame
static const int kFocusFrameWidth =  2;
                                    ...
                                    QFrame::Shadow shadow = QFrame::Plain)
{
    ...
    if (shadow != QFrame::Plain && (option->state & QStyle::State_HasFocus)) {
        ...
        border = defaultButton ? option->palette.highlight() : option->palette.shadow();
        qBrushSetAlphaF(&border, qreal(0.4));
        corner = defaultButton ? option->palette.highlight() : option->palette.shadow();
        qBrushSetAlphaF(&corner, 0.25);
        innerTopLeft = defaultButton ? option->palette.highlight() : option->palette.shadow();
        innerBottomRight = defaultButton ? option->palette.highlight() : option->palette.shadow();
...
/*!
    \class VanishStyle
    \brief The VanishStyle class provides a widget style similar to the
    Plastik style available in KDE.

    \inmodule QtWidgets

    The Vanish style provides a default look and feel for widgets on X11
...
*/
void VanishStyle::drawPartialFrame(QPainter *painter, const QStyleOptionComplex *option,
                                   const QRect &rect, const QWidget *widget) const
{
    ...
    frameOpt.rect = rect;
    painter->save();
    frameOpt.rect.adjust(-kFocusFrameWidth + (reverse ? 1 : 0), -kFocusFrameWidth,
                         kFocusFrameWidth + (reverse ? 0 : -1), kFocusFrameWidth);
...
/*!
  \reimp
*/
void VanishStyle::drawPrimitive(PrimitiveElement element, const QStyleOption *option,
                                QPainter *painter, const QWidget *widget) const
{
    ...
    switch (element) {
        case PE_IndicatorArrowUp:
            drawArrow(ArrowUp, option, painter, widget);
            break;
        case PE_IndicatorArrowDown:
            drawArrow(ArrowDown, option, painter, widget);
            break;
        case PE_IndicatorArrowLeft:
            drawArrow(ArrowLeft, option, painter, widget);
            break;
        case PE_IndicatorArrowRight:
            drawArrow(ArrowRight, option, painter, widget);
            break;
        ...
        case PE_PanelLineEdit:
            if (const QStyleOptionFrame *lineEdit = qstyleoption_cast<const QStyleOptionFrame *>(option)) {
                ...
                QRect filledRect = subElementRect(SE_LineEditContents, option, widget);
                QBrush baseBrush = qMapBrushToRect(lineEdit->palette.base(), filledRect);
                painter->setBrushOrigin(filledRect.topLeft());
                painter->fillRect(filledRect, baseBrush);

                // Inner border
                painter->setPen(shadeColor(lineEdit->palette.base().color(), 0.872));
                const QLine innerBorder[2] = {
                    QLine(filledRect.topLeft(), filledRect.topRight()),
                    QLine(filledRect.topLeft(), filledRect.bottomLeft())
                };
                painter->drawLines(innerBorder, 2);
        ...
        case PE_IndicatorHeaderArrow:
            if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option)) {
                ...
                if (flags &State_UpArrow || (header->sortIndicator == QStyleOptionHeader::SortUp))
                    orientation = ArrowUp;
                else if (flags &State_DownArrow || (header->sortIndicator == QStyleOptionHeader::SortDown))
                ...
                const QColor color = palette.windowText().color();
                ...
                painter->setBrush(QBrush(mergedColors(background, color)));
        ...
        case PE_IndicatorToolBarHandle: {
            ...
            drawDots(painter, option->rect, !(option->state & State_Horizontal), 2, 5, colors, -2, 1);
            break;
        }
        ...
        case PE_PanelButtonCommand:
            if (const QStyleOptionButton *button = qstyleoption_cast<const QStyleOptionButton *>(option)) {
                ...
                // Reserve some space for the default button border
                QRect fillRect = rect.adjusted(2, 2, -2, -2);

                // Fill background
                QLinearGradient buttonGradient(rect.topLeft(), rect.bottomLeft());
                if (sunken) {
                    buttonGradient.setColorAt(0.0, option->palette.button().color().darker(114));
                    buttonGradient.setColorAt(1.0, option->palette.button().color().darker(106));
                } else {
                    if (hover) {
                        buttonGradient.setColorAt(0.0, option->palette.button().color().lighter(114));
                        buttonGradient.setColorAt(1.0, option->palette.button().color().darker(100));
                    } else {
                        buttonGradient.setColorAt(0.0, option->palette.button().color().lighter(104));
                        buttonGradient.setColorAt(1.0, option->palette.button().color().darker(105));
                    }
                }
                p->fillRect(fillRect, qMapBrushToRect(buttonGradient, rect));

                // Left and top lines
                p->setPen(QPen(qBrushLight(buttonGradient, 105), 0));
                ...
                p->setPen(QPen(qBrushDark(buttonGradient, 105), 0));
        ...
        case PE_IndicatorCheckBox:
            if (const QStyleOptionButton *button = qstyleoption_cast<const QStyleOptionButton *>(option)) {
                ...
                // Outline
                QBrush border = on ? option->palette.highlight() : option->palette.shadow();
                qBrushSetAlphaF(&border, qreal(0.6));
                if (on)
                    qBrushDark(border, 220);
                ...
                QBrush corner = on ? option->palette.highlight() : option->palette.shadow();
                ...
                // Fill
                QBrush baseBrush = qMapBrushToRect(on ? button->palette.highlight() : button->palette.base(), rect);
                if (!baseBrush.gradient() && baseBrush.texture().isNull()) {
                    QLinearGradient gradient(rect.center().x(), rect.top(), rect.center().x(), rect.bottom());

                    if (on) {
                        gradient.setColorAt(0.0, baseBrush.color().lighter(190));
                        gradient.setColorAt(0.5, baseBrush.color().lighter(135));
                        gradient.setColorAt(1.0, baseBrush.color().lighter(190));
                    } else {
                        gradient.setColorAt(0.0, baseBrush.color());
                        gradient.setColorAt(1.0, baseBrush.color().darker(105));
                    }
                    baseBrush = gradient;
                }
                ...
                if (on || (enabled && sunken) || unchanged) {
                    ...
                    QColor color = mergedColors(button->palette.highlight().color(), button->palette.text().color());
                    if (sunken)
                        alphaColor(color, qreal(0.5));
                    else if (unchanged)
                        alphaColor(color, qreal(0.3));
                    p->setPen(QPen(color, 2));

                    if (unchanged) {
                        // Draw a tri-state mark
                        const QPointF p1(rect.left(), rect.center().y());
                        const QPointF p2(rect.right(), rect.center().y());

                        p->drawLine(p1, p2);
                    } else {
                        // Draw a normal check mark
                        const qreal x(rect.center().x() - 2.5);
                        const qreal y(rect.center().y() - 5.5);
        ...
        case PE_IndicatorRadioButton:
            if (const QStyleOptionButton *button = qstyleoption_cast<const QStyleOptionButton *>(option)) {
                ...
                QBrush border = qMapBrushToRect(on ? option->palette.highlight() : option->palette.shadow(), rect);
                qBrushSetAlphaF(&border, qreal(0.6));
                if (on)
                    qBrushDark(border, 220);
                p->setPen(QPen(border, 0));
                ...
                if (on || (enabled && sunken)) {
                    ...
                    p->drawEllipse(QRectF(rect).adjusted(6, 6, -6, -6));
                }
...
/*!
  \reimp
*/
void VanishStyle::drawControl(ControlElement element, const QStyleOption *option,
                              QPainter *painter, const QWidget *widget) const
{
    QColor borderColor = option->palette.background().color().darker(178);
    QColor alphaCornerColor;
    ...
    switch (element) {
        ...
        case CE_MenuBarItem:
...
#else
                    if ((option->state & QStyle::State_Sunken) || (option->state & QStyle::State_On)) {
                        cachePainter.setPen(QPen(option->palette.highlight().color().dark(120), 0));
                        cachePainter.drawRect(rect);
                        cachePainter.fillRect(rect, option->palette.highlight());
                    }
#endif
                    cachePainter.end();
                    QPixmapCache::insert(pixmapName, cache);
            ...
            if (const QStyleOptionMenuItem *mbi = qstyleoption_cast<const QStyleOptionMenuItem *>(option)) {
                ...
                // Use window text (or highlighted text if selected) color instead of button text
                if (!(option->palette.resolve() & (1 << QPalette::ButtonText))) {
                    if ((option->state & QStyle::State_Sunken) || (option->state & QStyle::State_On))
                ...
                // Make background transparent to not cover the background
                if (!(option->state & State_Selected)) {
                    newMI.palette.setColor(QPalette::Window, Qt::transparent);
                    newMI.palette.setColor(QPalette::Button, Qt::transparent);
                }
        ...
        case CE_MenuBarEmptyArea:
            if (widget && qobject_cast<const QMainWindow *>(widget->parentWidget())) {
                const QColor background = option->palette.window().color();

                QLinearGradient gradient(option->rect.topLeft(), option->rect.bottomLeft());
                gradient.setColorAt(0.0, shadeColor(background, 0.94));
                gradient.setColorAt(1.0, shadeColor(background, 0.90));
        ...
        case CE_ToolBar:
            if (const QStyleOptionToolBar *toolBar = qstyleoption_cast<const QStyleOptionToolBar *>(option)) {
                qreal startFrom = 0.90;

                // Start from menubar starting point if there's no menubar
                const QMainWindow *mainWindow = qobject_cast<const QMainWindow *>(widget);
                if (mainWindow && (!mainWindow->menuBar() || (mainWindow->menuBar() && !mainWindow->menuBar()->isVisible())))
                    startFrom = 0.94;

                // Background gradient
                QColor backgroundColor = option->palette.background().color();
                QLinearGradient gradient(option->rect.topLeft(), option->rect.bottomLeft());
                switch (toolBar->positionOfLine) {
                    case QStyleOptionToolBar::OnlyOne:
                    case QStyleOptionToolBar::Beginning:
                        gradient.setColorAt(0.0, shadeColor(backgroundColor, startFrom));
                        gradient.setColorAt(1.0, shadeColor(backgroundColor, startFrom - 0.04));
                        break;
                    case QStyleOptionToolBar::Middle:
                        gradient.setColorAt(0.0, shadeColor(backgroundColor, startFrom - 0.04));
                        gradient.setColorAt(1.0, shadeColor(backgroundColor, startFrom - 0.06));
                        break;
                    case QStyleOptionToolBar::End:
                        gradient.setColorAt(0.0, shadeColor(backgroundColor, startFrom - 0.06));
                        gradient.setColorAt(1.0, shadeColor(backgroundColor, startFrom - 0.04));
                        break;
                }
                ...
                // Draw line for the last tool bar only
                if (toolBar->positionOfLine == QStyleOptionToolBar::OnlyOne ||
                        toolBar->positionOfLine == QStyleOptionToolBar::End) {
                    QPen oldPen = painter->pen();
                    painter->setPen(shadeColor(backgroundColor, 0.623));
        ...
        case CE_ScrollBarAddLine:
            if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option)) {
                ...
                if (!QPixmapCache::find(addLinePixmapName, cache)) {
                    ...
                    // Arrow
                    if (horizontal) {
                        drawArrow(reverse ? ArrowLeft : ArrowRight, option, painter, widget);
#if 0
        ...
        case CE_ScrollBarSubLine:
            if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option)) {
                ...
                if (!QPixmapCache::find(subLinePixmapName, cache)) {
                    ...
                    // Arrows
                    if (horizontal) {
                        drawArrow(reverse ? ArrowRight : ArrowLeft, option, painter, widget);
#if 0
        ...
        case CE_ComboBoxLabel:
            if (const QStyleOptionComboBox *comboBox = qstyleoption_cast<const QStyleOptionComboBox *>(option)) {
                ...
                if (!comboBox->editable) {
                    // Non-editable combo box is drawn as a button, so we need
                    // the label to be drawn using ButtonText where it would
                    // usually use Text
...
/*!
  \reimp
*/
void VanishStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex *option,
                                     QPainter *painter, const QWidget *widget) const
{
    QColor borderColor = option->palette.background().color().darker(178);
    QColor alphaCornerColor;
        ...
        case CC_ComboBox:
            if (const QStyleOptionComboBox *comboBox = qstyleoption_cast<const QStyleOptionComboBox *>(option)) {
                ...
                if (comboBox->editable) {
                    ...
                    // ComboBox starts with a line edit in place already
                    QRect buttonRect;
                    if (!reverse)
                ...
                if (comboBox->subControls & SC_ComboBoxArrow) {
                    ...
                    painter->setRenderHint(QPainter::Antialiasing);
                    painter->setPen(QPen(option->palette.buttonText(), penThickness, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
...
/*!
  \reimp
*/
QSize VanishStyle::sizeFromContents(ContentsType type, const QStyleOption *option,
...
/*!
  \reimp
*/
QRect VanishStyle::subControlRect(ComplexControl control, const QStyleOptionComplex *option,
            ...
            if (const QStyleOptionSpinBox *spinBox = qstyleoption_cast<const QStyleOptionSpinBox *>(option)) {
                ...
                switch (subControl) {
                    ...
                    case SC_SpinBoxEditField:
                        ...
                        rect.adjust(kFocusFrameWidth, kFocusFrameWidth, -kFocusFrameWidth, -kFocusFrameWidth);
                        rect = visualRect(spinBox->direction, spinBox->rect, rect);
                        break;
                    default:
                        break;
            ...
            switch (subControl) {
                ...
                case SC_ComboBoxEditField: {
                    if (const QStyleOptionComboBox *box = qstyleoption_cast<const QStyleOptionComboBox *>(option)) {
                        ...
                        if (box->editable) {
                            rect = box->rect.adjusted(kFocusFrameWidth, kFocusFrameWidth, -kFocusFrameWidth, -kFocusFrameWidth);
...
/*!
  \reimp
*/
int VanishStyle::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget,
...
/*!
  \reimp
*/
QStyle::SubControl VanishStyle::hitTestComplexControl(ComplexControl control, const QStyleOptionComplex *option,
...
/*!
  \reimp
*/
int VanishStyle::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
{
    int ret = -1;

    switch (metric) {
            // TODO: Use settings to dermine icons size
        case PM_SmallIconSize:
            return int(dpiScaled(16.));
        case PM_LargeIconSize:
            return int(dpiScaled(48.));
        case PM_ToolBarIconSize:
            return int(dpiScaled(24.));
        ...
        case PM_ButtonIconSize:
            return proxy()->pixelMetric(PM_ToolBarIconSize, option, widget);
...
QPalette VanishStyle::standardPalette() const
{
    ...
    palette.setBrush(QPalette::Disabled, QPalette::Window, QColor(QRgb(0xfff4f4f2)));
...
/*!
    \internal
*/
QIcon VanishStyle::standardIconImplementation(StandardPixmap standardIcon, const QStyleOption *option,
...
/*!
    \reimp
*/
QPixmap VanishStyle::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt,
                                    const QWidget *widget) const
{
...
/*!
    \internal
*/
int VanishStyle::layoutSpacingImplementation(QSizePolicy::ControlType control1,
