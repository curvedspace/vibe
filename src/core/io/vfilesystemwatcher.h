...
#ifndef VFILESYSTEMWATCHER_H
#define VFILESYSTEMWATCHER_H
...
class VFileSystemWatcherPrivate;

/**
 * \class VFileSystemWatcher vfilesystemwatcher.h <VFileSystemWatcher>
 *
 ...
 * created or deleted, VFileSystemWatcher will emit the signal dirty().
 *
 * When a watched, but previously not existing directory gets created,
 * VFileSystemWatcher will emit the signal created().
 *
 * When a watched directory gets deleted, VFileSystemWatcher will emit the
 * signal deleted(). The directory is still watched for new
 * creation.
 *
 * When a watched file is changed, i.e. attributes changed or written
 * to, VFileSystemWatcher will emit the signal dirty().
 *
...
public:
    ...
    /**
     * Dump statistic information about the VFileSystemWatcher::self() instance.
     * This checks for consistency, too.
     */
    static void statistics(); // TODO implement a QDebug operator for VFileSystemWatcher instead.

    /**
     * The VFileSystemWatcher instance usually globally used in an application.
     * It is automatically deleted when the application exits.
     *
     * However, you can create an arbitrary number of VFileSystemWatcher instances
...
private:
    VFileSystemWatcherPrivate *const d;
};

Q_DECLARE_OPERATORS_FOR_FLAGS(VFileSystemWatcher::WatchModes)

/** @}*/

#endif // VFILESYSTEMWATCHER_H
